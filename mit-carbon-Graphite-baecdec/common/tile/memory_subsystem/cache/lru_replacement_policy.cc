...
vector<UInt32>
LRUReplacementPolicy::getReplacementWay(CacheLineInfo** cache_line_info_array, UInt32 set_num,/*ShmemMsg::*/PrL1ShL2MSI:: ShmemMsg* shmem_msg )
{
   vector<UInt32> s;
...
void
LRUReplacementPolicy::update(CacheLineInfo** cache_line_info_array, UInt32 set_num, vector<UInt32> accessed_way)
{
   vector<UInt8>& lru_bits = _lru_bits_vec[set_num];
   for (UInt32 i = 0; i < _associativity; i++)
   {
