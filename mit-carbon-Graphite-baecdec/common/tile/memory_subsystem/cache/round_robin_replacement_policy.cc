...
vector<UInt32>
RoundRobinReplacementPolicy::getReplacementWay(CacheLineInfo** cache_line_info_array, UInt32 set_num,/*ShememMsg::*/PrL1ShL2MSI:: ShmemMsg* shmem_msg)
{
   vector<UInt32> s;
   ...
   _replacement_index_vec[set_num] = (_replacement_index_vec[set_num] == 0) ?
    ...
    s.push_back(curr_replacement_index);
    return s;
...
void
RoundRobinReplacementPolicy::update(CacheLineInfo** cache_line_info_array, UInt32 set_num, vector<UInt32> accessed_way)
{
   return;
}
void
