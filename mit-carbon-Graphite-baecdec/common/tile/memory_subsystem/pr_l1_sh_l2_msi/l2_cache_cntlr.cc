...
{
UInt32 L2CacheCntlr::_comp_cntr;
   ...
   , _enabled(false)
{
   ...
   _dm_control = dmcontrol;
   // L2 cache  
   _L2_cache_replacement_policy_obj /*l2_crp*/ = new L2CacheReplacementPolicy(L2_cache_size, L2_cache_associativity, cache_line_size, _L2_cache_req_queue/*,_dm_control,_L2_cache*/);
   //_L2_cache_replacement_policy_obj->setdc(_dm_control);
   _L2_cache = new CompressedCache("L2",
         PR_L1_SH_L2_MSI,
         //Cache::UNIFIED_CACHE,
         CompressedCache::COMPRESSED_CACHE,
         L2,
         CompressedCache::WRITE_BACK,
         L2_cache_size, 
     ...
     //_L2_cache_replacement_policy_obj->setccp(_L2_cache);
     //_L2_cache_replacement_policy_obj->setdc(_dm_control);
    L2CacheReplacementPolicy* l2_crp = dynamic_cast<L2CacheReplacementPolicy*>(_L2_cache_replacement_policy_obj);
    l2_crp->setdc(_dm_control);
    l2_crp->setccp(_L2_cache); 
...
void
L2CacheCntlr::getCacheLineInfo(IntPtr address, ShL2CacheLineInfo* L2_cache_line_info, ShmemMsg::Type shmem_msg_type, bool update_miss_counters , /*ShememMsg::*/ShmemMsg* shmem_msg)
{
   //LOG_PRINT_WARNING("get into getcachelineinfo \n");
   ...
   if (it == _evicted_cache_line_map.end())
   {
      ...
      if (L2_cache_line_info->getCState() == CacheState::INVALID)
      {
         if ((shmem_msg_type == ShmemMsg::EX_REQ) || (shmem_msg_type == ShmemMsg::SH_REQ))
         {
            ...
            allocateCacheLine(address, L2_cache_line_info,shmem_msg);
         }
         else
         {
...
{
  _L2_cache->accessCacheLine(address, CompressedCache::LOAD, data_buf, getCacheLineSize());
}

void
L2CacheCntlr::writeCacheLine(IntPtr address, Byte* data_buf)
{
   _L2_cache->accessCacheLine(address, CompressedCache::STORE, data_buf, getCacheLineSize());
}

void
L2CacheCntlr::allocateCacheLine(IntPtr address, ShL2CacheLineInfo* L2_cache_line_info,/*ShememMsg::*/ShmemMsg* shmem_msg)
{
   ...
   *L2_cache_line_info = ShL2CacheLineInfo(_L2_cache->getTag(address), directory_entry);
   ...
   int eviction=0;  // boundary condition
   IntPtr evicted_address1;
   IntPtr evicted_address2;
   ShL2CacheLineInfo evicted_cache_line_info1;
   ShL2CacheLineInfo evicted_cache_line_info2;
   Byte writeback_buf1[getCacheLineSize()];
   Byte writeback_buf2[getCacheLineSize()];
   //LOG_PRINT_WARNING("get into allocatecacheline \n");//////
   _L2_cache->insertCacheLine(address, L2_cache_line_info, /*shmem_msg->getDataBuf()*/(Byte*) NULL,
                              &eviction, &evicted_address1, &evicted_address2,&evicted_cache_line_info1,&evicted_cache_line_info2, writeback_buf1,writeback_buf2,shmem_msg);
   ...
   {
      _comp_cntr=0;
      _L2_cache->calcCompRatio();
   } //////////////////////////////////
   assert(eviction>=0&&eviction<=2);
   ...
   if (eviction)
   {
      ...
      assert(evicted_cache_line_info1.isValid());
      LOG_ASSERT_ERROR(_L2_cache_req_queue.empty(evicted_address1),
                       "Address(%#lx) is already being processed", evicted_address1);
      LOG_ASSERT_ERROR(evicted_cache_line_info1.getCState() == CacheState::CLEAN || evicted_cache_line_info1.getCState() == CacheState::DIRTY,
                       "Cache Line State(%u)", evicted_cache_line_info1.getCState());
      __attribute__((unused)) DirectoryEntry* evicted_directory_entry1 = evicted_cache_line_info1.getDirectoryEntry();
      LOG_ASSERT_ERROR(evicted_directory_entry1, "Cant find directory entry for address1(%#lx)", evicted_address1);

      bool msg_modeled = Config::getSingleton()->isApplicationTile(getTileId());
      Time eviction_time1 = getShmemPerfModel()->getCurrTime();
      
      LOG_PRINT("Eviction: Address(%#lx), Cache State(%u), Directory State(%u), Num Sharers(%i)",
                evicted_address1, evicted_cache_line_info1.getCState(),
                evicted_directory_entry1->getDirectoryBlockInfo()->getDState(), evicted_directory_entry1->getNumSharers());

      // Create a nullify req and add it onto the queue for processing
      ShmemMsg nullify_msg1(ShmemMsg::NULLIFY_REQ, MemComponent::L2_CACHE, MemComponent::L2_CACHE,
                           getTileId(), false, evicted_address1,
                           msg_modeled); 
      // Create a new ShmemReq for removing the sharers of the evicted cache line
      ShmemReq* nullify_req1 = new ShmemReq(&nullify_msg1, eviction_time1);
      // Insert the nullify_req into the set of requests to be processed
      _L2_cache_req_queue.enqueue(evicted_address1, nullify_req1);
      
      // Insert the evicted cache line info into the evicted cache line map for future reference
      _evicted_cache_line_map.insert(make_pair(evicted_address1, evicted_cache_line_info1));
     
      // Process the nullify req
      processNullifyReq(nullify_req1, writeback_buf1);
      }
      /*if(eviction == 2)
      {
      assert(evicted_cache_line_info1.isValid());
      ...
      assert(evicted_cache_line_info2.isValid());
      LOG_ASSERT_ERROR(_L2_cache_req_queue.empty(evicted_address2),
                       "Address(%#lx) is already being processed", evicted_address2);
      LOG_ASSERT_ERROR(evicted_cache_line_info2.getCState() == CacheState::CLEAN || evicted_cache_line_info2.getCState() == CacheState::DIRTY,
                       "Cache Line State(%u)", evicted_cache_line_info2.getCState());
      __attribute__((unused)) DirectoryEntry* evicted_directory_entry2 = evicted_cache_line_info2.getDirectoryEntry();
      LOG_ASSERT_ERROR(evicted_directory_entry2, "Cant find directory entry for address2(%#lx)", evicted_address2);

      //bool msg_modeled = Config::getSingleton()->isApplicationTile(getTileId());
      Time eviction_time2 = getShmemPerfModel()->getCurrTime();
      
      LOG_PRINT("Eviction: Address(%#lx), Cache State(%u), Directory State(%u), Num Sharers(%i)",
                evicted_address2, evicted_cache_line_info2.getCState(),
                evicted_directory_entry2->getDirectoryBlockInfo()->getDState(), evicted_directory_entry2->getNumSharers());

      // Create a nullify req and add it onto the queue for processing
      ShmemMsg nullify_msg2(ShmemMsg::NULLIFY_REQ, MemComponent::L2_CACHE, MemComponent::L2_CACHE,
                           getTileId(), false, evicted_address2,
                           msg_modeled); 
      // Create a new ShmemReq for removing the sharers of the evicted cache line
      ShmemReq* nullify_req2 = new ShmemReq(&nullify_msg2, eviction_time2);
      // Insert the nullify_req into the set of requests to be processed
      _L2_cache_req_queue.enqueue(evicted_address2, nullify_req2);
      
      // Insert the evicted cache line info into the evicted cache line map for future reference
      _evicted_cache_line_map.insert(make_pair(evicted_address2, evicted_cache_line_info2));
     
      // Process the nullify req
      processNullifyReq(nullify_req2, writeback_buf2);
      }*/
   }
   else
   {
      assert(!evicted_cache_line_info1.isValid());
      assert(evicted_cache_line_info1.getDirectoryEntry() == NULL);
   }
}
...
L2CacheCntlr::handleMsgFromL1Cache(tile_id_t sender, ShmemMsg* shmem_msg)
{
   ...
   /*if((shmem_msg_type == ShmemMsg::EX_REQ) || (shmem_msg_type == ShmemMsg::SH_REQ))
   {
   cr = L2_cache_line_info_t.getcomsize();
   if (cr<= 4)  // fixed threshold 
   _memory_manager->incrCurrTime(MemComponent::L2_CACHE, CachePerfModel::WUNCOMPRESSED); // Here can be applied a bypass according to the cache line whether being compressed
   else
   _memory_manager->incrCurrTime(MemComponent::L2_CACHE, CachePerfModel::UNCOMPRESSED); 
   }*/
   if((shmem_msg_type == ShmemMsg::EX_REQ) || (shmem_msg_type == ShmemMsg::SH_REQ))
   {
   //cr = L2_cache_line_info_t.getcomsize();
   ...
   _memory_manager->incrCurrTime(MemComponent::L2_CACHE, CachePerfModel::ACCESS_DATA_AND_TAGS); //// add uncompressed time
   ...
   else if ( (shmem_msg_type == ShmemMsg::INV_REP) || (shmem_msg_type == ShmemMsg::FLUSH_REP) || (shmem_msg_type == ShmemMsg::WB_REP) )
   {
      ...
      getCacheLineInfo(address, &L2_cache_line_info, shmem_msg_type,updatemiss, shmem_msg);
...
void
L2CacheCntlr::handleMsgFromDram(tile_id_t sender, ShmemMsg* shmem_msg)   // further processing
{
...
L2CacheCntlr::processNullifyReq(ShmemReq* nullify_req, Byte* data_buf)
{
   ...
   // get cache line info 
   ShL2CacheLineInfo L2_cache_line_info;
   getCacheLineInfo(address, &L2_cache_line_info, ShmemMsg::NULLIFY_REQ, updatemiss,nullify_req->getShmemMsg());
...
L2CacheCntlr::processExReqFromL1Cache(ShmemReq* shmem_req, Byte* data_buf, bool first_call)
{
   ...
   ShL2CacheLineInfo L2_cache_line_info;
   getCacheLineInfo(address, &L2_cache_line_info, ShmemMsg::EX_REQ, first_call,shmem_req->getShmemMsg());
...
L2CacheCntlr::processShReqFromL1Cache(ShmemReq* shmem_req, Byte* data_buf, bool first_call)
{
   ...
   ShL2CacheLineInfo L2_cache_line_info;
   getCacheLineInfo(address, &L2_cache_line_info, ShmemMsg::SH_REQ, first_call,shmem_req->getShmemMsg());
